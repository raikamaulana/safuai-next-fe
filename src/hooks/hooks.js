...
export const useFarmList = () => {
  const [{ response, isLoading, errResponse }, submit] = useGet(
    getEnv().FARMING_API + "/api/v1/farms",
    false
  );
...
export const useFarmDetail = (id) => {
  const [{ response, isLoading, errResponse }, submit] = useGet(
    getEnv().FARMING_API + "/api/v1/farms/" + id,
    true
  );
  ...
  useEffect(() => {
    ...
    if (response.data && response.data.code === 200)
      setFarmDetail(response.data.data[0]);
  }, [response]);
...
export const usePricePerToken = (tokenA, tokenB) => {
  ...
  const updatePricePerToken = async () => {
    const router = await new state.web3.eth.Contract(
      getAppByChainId(state.chainId).ROUTER_ABI,
      state.web3.utils.toHex(getAppByChainId(state.chainId).ROUTER_ADDRESS)
    );
    const path = [
      state.web3.utils.toHex(tokenA),
      state.web3.utils.toHex(tokenB),
    ];
    const estimate = await router.methods
      .getAmountsOut(state.web3.utils.toWei("1"), path)
      .call();
