...
const OverviewInformation = dynamic(
  () => import("@/components/OverviewInformation").then((mod) => mod.OverviewInformation),
  {
    ssr: false,
  }
);
...
const ContractInheritance = dynamic(
  () => import("@/components/contractinheritance").then((mod) => mod.ContractInheritance),
  {
    ssr: false,
  }
);
...
const WebsiteInformation = dynamic(
  () => import("@/components/WebsiteInformation").then((mod) => mod.WebsiteInformation),
  {
    ssr: false,
  }
);
    ...
    <section className='bg-[#E8ECEF] dark:bg-[#0D1116] duration-500 transition-all'>
      ...
      <Tab.Group vertical>
              <Tab.List className="flex flex-col bg-white dark:text-white dark:bg-[#161C23] text-left w-80  mr-5 px-3 pt-4 min-h-max">
                ...
                <input type='text' className='border-[0.5px] w-[99%] border-[#E8ECEF] bg-white dark:bg-[#2C3239] dark:border-none rounded-lg mb-3 placeholder:text-[#5E656C] pl-11' placeholder='Search'/>
                <svg width="20" height="20" className='absolute top-3 left-3' viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M8.86088 0C13.7521 0 17.7218 3.96968 17.7218 8.86088C17.7218 13.7521 13.7521 17.7218 8.86088 17.7218C3.96968 17.7218 0 13.7521 0 8.86088C0 3.96968 3.96968 0 8.86088 0ZM8.86088 15.7527C12.6681 15.7527 15.7527 12.6681 15.7527 8.86088C15.7527 5.05267 12.6681 1.96909 8.86088 1.96909C5.05267 1.96909 1.96909 5.05267 1.96909 8.86088C1.96909 12.6681 5.05267 15.7527 8.86088 15.7527ZM17.2147 15.8226L20 18.6069L18.6069 20L15.8226 17.2147L17.2147 15.8226Z" fill="#9098A0"/>
                </svg>
                </div>
                <Tab as={Fragment}>
                  {({ selected }) => (
                    /* Use the `selected` state to conditionally style the selected tab. */
                    <button className={selected ? 'text-left h-12 pl-1 pr-5 bg-[#E8ECEF] dark:bg-[#20262D] border-l-8 border-[#D171FF]' : 'text-left h-12 pl-3 pr-5'}>
                      Tax / Fee Information
                    </button>
                  )}
                </Tab>
